...
void Missile :: Spawn(Entity **Mstart, int Velocity, int Health) { // Makes the missile initial pointer the return
	PlayerPos = Spawn.getPos();
	PlayerBox = Spawn.getBox();
	Mend = NULL;
	if ( (*Mstart) == NULL ) { // No projectiles created yet
		*Mstart = (Missile *) malloc (sizeof(Entity));
		...
		Mend -> next = (Missile *) malloc (sizeof(Entity));
	...
	Mend -> box.x  = 16 * Mend -> direction;
	Mend -> Speed = Velocity // Sets speed and health of each individual missile;
	Mend -> Health = Health;
}
